...
@Component({
  ...
  template: `
    ...
    <p class="mt-1 text-sm text-cacao/60">
      Pedidos del carrito y consultas recibidas desde la landing.
    </p>
        ...
        <li class="rounded-vitrina bg-white p-4 shadow-bordo sm:p-5">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <p class="font-display font-bold text-bordo">
              ...
              @if (!pedido.items) {
                <span
                  class="ml-1 rounded-full bg-cacao/10 px-2 py-0.5 text-xs font-normal text-cacao/60"
                  >Consulta</span
                >
              }
            </p>
            <div class="flex items-center gap-3">
              @if (pedido.creado_en) {
                <span class="text-xs text-cacao/50">{{
          ...
          @if (pedido.items; as items) {
              ...
              @for (item of items; track item.nombre) {
                <li class="flex justify-between gap-3">
                  ...
                  <span class="font-bold">{{
                    item.precio * item.cantidad | currency: 'UYU' : '$ ' : '1.0-0'
                  }}</span>
                </li>
              }
            ...
            <dl class="mt-3 grid gap-x-6 gap-y-1 text-sm text-cacao/80 sm:grid-cols-2">
              <div>
                ...
                <dd class="inline">{{ pedido.email }}</dd>
              </div>
              <div>
                <dt class="inline font-bold">Teléfono:</dt>
                <dd class="inline">{{ pedido.telefono }}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd class="inline">
                  {{ pedido.pago === 'transferencia' ? 'Transferencia' : 'Efectivo' }}
                </dd>
              </div>
              @if (pedido.direccion) {
                <div class="sm:col-span-2">
                  <dt class="inline font-bold">Dirección / punto:</dt>
                  <dd class="inline">{{ pedido.direccion }}</dd>
                </div>
              }
              ...
              @if (pedido.preferencias) {
                <div class="sm:col-span-2">
                  <dt class="inline font-bold">Preferencias:</dt>
                  <dd class="inline">{{ pedido.preferencias }}</dd>
                </div>
              }
            </dl>
          } @else {
