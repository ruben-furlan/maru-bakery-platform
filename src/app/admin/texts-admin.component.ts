...
@Component({
  ...
  template: `
    ...
    <p class="mt-1 text-sm text-cacao/60">Editá el eslogan, el marquee y los datos de contacto sin tocar código.</p>
    ...
    <form class="mt-6 max-w-2xl space-y-5 rounded-vitrina bg-white p-5 shadow-bordo sm:p-6" (ngSubmit)="guardar()">
      ...
      <label class="block">
        ...
        <textarea name="hero_subtitulo" [(ngModel)]="form.hero_subtitulo" rows="2" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5"></textarea>
      </label>

      <label class="block">
        <span class="mb-1 block text-sm font-bold">Frases del marquee</span>
        <input type="text" name="marquee" [(ngModel)]="form.marquee" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5" />
      ...
      <div class="grid gap-5 sm:grid-cols-2">
        <label class="block">
          ...
          <input type="text" name="whatsapp" [(ngModel)]="form.whatsapp" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5" />
          <span class="mt-1 block text-xs text-cacao/50">Formato internacional sin "+", ej: 59899123456</span>
        </label>
        <label class="block">
          <span class="mb-1 block text-sm font-bold">Instagram</span>
          <input type="text" name="instagram" [(ngModel)]="form.instagram" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5" />
          <span class="mt-1 block text-xs text-cacao/50">Usuario sin "&#64;", ej: marubakery.uy</span>
        </label>
        <label class="block">
          <span class="mb-1 block text-sm font-bold">Email de contacto</span>
          <input type="email" name="email" [(ngModel)]="form.email" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5" />
        </label>
        <label class="block">
          <span class="mb-1 block text-sm font-bold">Dirección / zona</span>
          <input type="text" name="direccion" [(ngModel)]="form.direccion" class="w-full rounded-xl border border-cacao/20 px-4 py-2.5" />
        </label>
      </div>
