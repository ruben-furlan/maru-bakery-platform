...
@Component({
  ...
  template: `
    @if (carrito.abierto()) {
      <!-- Fondo oscurecido -->
      <div
        class="fixed inset-0 z-50 bg-cacao/60 backdrop-blur-sm"
        (click)="cerrar()"
        aria-hidden="true"
      ></div>
      ...
      >
                  ...
                  @for (item of carrito.items(); track item.producto.id) {
                    <li class="flex items-center gap-3 rounded-2xl bg-white p-3 shadow-bordo">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <span class="w-5 text-center font-bold" aria-live="polite">{{
                          item.cantidad
                        }}</span>
                        <button
                          type="button"
                          ...
                          [attr.title]="
                            item.cantidad >= maxPorProducto
                              ? 'Máximo ' + maxPorProducto + ' por producto'
                              : null
                          "
              ...
              <form id="form-checkout" class="space-y-4" (ngSubmit)="confirmar()">
                ...
                <label class="block text-sm">
                  ...
                  <span class="mt-1 block text-xs text-cacao/60"
                    >Te mandamos el resumen del pedido a este correo.</span
                  >
                </label>
                ...
                <fieldset>
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <label
                      ...
                      [class]="
                        entrega === 'envio' ? 'border-bordo bg-bordo text-crema' : 'border-bordo/20'
                      "
                    >
                      <input
                        type="radio"
                ...
                <fieldset>
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    ...
                    <label
                      class="cursor-pointer rounded-xl border-2 p-3 text-center text-sm font-bold transition-colors"
                      [class]="
                        pago === 'efectivo' ? 'border-bordo bg-bordo text-crema' : 'border-bordo/20'
                      "
                    >
                      <input
                        type="radio"
                ...
                <div
                  class="flex items-start gap-3 rounded-2xl border border-dorado/50 bg-dorado/10 p-4"
                >
                  <span class="text-2xl" aria-hidden="true">🗓️</span>
                  <p class="text-sm leading-relaxed text-cacao/80">
                    <span class="font-script text-base text-bordo">¿Y el día de entrega?</span
                    ><br />
                    Cuando confirmemos tu pedido coordinamos juntos el día y la hora que mejor te
                    quede. Horneamos todo fresquito, por encargo 💛
                  </p>
                </div>

                <label class="block text-sm">
                  <span class="mb-1 block font-bold text-cacao/80"
                    >¿Tenés alguna preferencia? Contanos</span
                  >
                  <textarea
                    name="preferencias"
                ...
                @if (error()) {
                  <p class="rounded-xl bg-bordo/10 p-3 text-sm text-bordo" role="alert">
                    {{ error() }}
                  </p>
                }
              </form>
            ...
            @case ('listo') {
              <div class="py-8 text-center">
                ...
                <p class="mx-auto mt-3 max-w-xs text-cacao/70">
                  ...
                  >. En breve nos ponemos en contacto para confirmar todo y coordinar juntos el día
                  de entrega.
                </p>
              </div>
            }
        ...
        <footer
          class="border-t border-bordo/10 px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4"
        >
          @switch (paso()) {
            @case ('carrito') {
              @if (carrito.items().length > 0) {
            ...
            @case ('datos') {
              <div class="mb-3 flex items-center justify-between">
                <button
                  type="button"
                  (click)="paso.set('carrito')"
                  class="text-sm text-bordo underline"
                >
                  ← Volver al carrito
                </button>
                <span class="font-display text-xl font-bold text-bordo">
...
export class CartComponent {
  ...
  async confirmar(): Promise<void> {
    ...
    if (error) {
      this.error.set(
        'No pudimos registrar tu pedido. Probá de nuevo en unos minutos o escribinos por WhatsApp.',
      );
      return;
    }
